...
function _via_util_file_select_local(type, handler, multiple) {
  var fsel;
  if(type === _VIA_FILE_SELECT_TYPE.TEXT || type ===_VIA_FILE_SELECT_TYPE.JSON || !confirm("Загрузить директорию целиком?") ) {
    fsel = document.createElement('input');
    ...
    fsel = document.getElementById('InputDir');
  }
